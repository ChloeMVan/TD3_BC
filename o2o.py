...
if __name__ == "__main__":
	...
	parser.add_argument("--max_timesteps", default=5000_000, type=int)   # Max time steps to run environment
	...
	for t in range(int(args.max_timesteps)):
		...
		if (t + 1) % args.eval_freq == 0:
			...
			evaluations.append(eval_offline_policy(policy, args.env, args.seed, mean, std))
	...
	evaluations = [eval_online_policy(td3policy, args.env, args.seed)]
	...
	for t in range(int(args.max_timesteps)):
		...
		online_buffer.add(state, action, next_state, reward, done)
		...
		# Evaluate episode
		if (t + 1) % args.eval_freq == 0:
			evaluations.append(eval_offline_policy(td3policy, args.env, args.seed, mean, std))
			np.save(f"./results/{file_name}", evaluations)
